...
class Xception_Adv:
    def get_enhanced_xception(input_tensor):
        ...
        ##### Entry_flow #####
        x = layers.Conv2D(32, (3, 3), strides=(2, 2), use_bias=False, padding='same', name='block1_conv1')(img_input)
        ...
        x = layers.DepthwiseConv2D((3, 3), strides=(2, 2), padding='same', use_bias=False, name='block2_depthwise')(x)
        x = layers.BatchNormalization(name='block2_depthwise_bn')(x)
        x = layers.Activation('relu')(x)
        x = layers.Conv2D(128, (1, 1), padding='same', use_bias=False, name='block2_pointwise')(x)
        x = layers.BatchNormalization(name='block2_pointwise_bn')(x)
        x = layers.Activation('relu')(x)
        # x = layers.Conv2D(128, (3, 3), strides=(2, 2), padding='same', use_bias=False, name='block2_alt')(x) # 假如用这个，参数多了好几倍
        ...
        x = layers.DepthwiseConv2D((3, 3), strides=(2, 2), padding='same', use_bias=False, name='block3_depthwise')(x)
        x = layers.BatchNormalization(name='block3_depthwise_bn')(x)
        x = layers.Activation('relu')(x)
        x = layers.Conv2D(256, (1, 1), padding='same', use_bias=False, name='block3_pointwise')(x)
        x = layers.BatchNormalization(name='block3_pointwise_bn')(x)
        x = layers.Activation('relu')(x)
        ...
        x = layers.DepthwiseConv2D((3, 3), strides=(2, 2), padding='same', use_bias=False, name='block4_depthwise')(x)
        x = layers.BatchNormalization(name='block4_depthwise_bn')(x)
        x = layers.Activation('relu')(x)
        x = layers.Conv2D(728, (1, 1), padding='same', use_bias=False, name='block4_pointwise')(x)
        x = layers.BatchNormalization(name='block4_pointwise_bn')(x)
        x = layers.Activation('relu')(x)
        ...
        x = layers.Activation('relu', name='block21_sepconv1_act')(x)
        x = layers.SeparableConv2D(728, (3, 3), padding='same', use_bias=False, name='block21_sepconv1')(x)
        x = layers.BatchNormalization(name='block21_sepconv1_bn')(x)
        x = layers.Activation('relu', name='block21_sepconv2_act')(x)
        x = layers.SeparableConv2D(1024, (3, 3), padding='same', use_bias=False, name='block21_sepconv2')(x)
        x = layers.BatchNormalization(name='block21_sepconv2_bn')(x)

        # x = layers.MaxPooling2D((3, 3), strides=(2, 2), padding='same', name='block21_pool')(x)
        x = layers.DepthwiseConv2D((3, 3), strides=(2, 2), padding='same', use_bias=False, name='block21_depthwise')(x)
        x = layers.BatchNormalization(name='block21_depthwise_bn')(x)
        x = layers.Activation('relu')(x)
        x = layers.Conv2D(1024, (1, 1), padding='same', use_bias=False, name='block21_pointwise')(x)
        x = layers.BatchNormalization(name='block21_pointwise_bn')(x)
        x = layers.Activation('relu')(x)
...
class DeepLabV3Plus:
    def get_model(input_shape=(513, 513, 3), class_no=21):
        ...
        with tf.variable_scope("encoder"):
            ...
            #TODO aspp4 需要检查
            aspp4 = layers.Conv2D(256, (1, 1), padding='same', use_bias=False, name='image_pooling')(x_output)  # 减少层数
            aspp4 = layers.BatchNormalization(name='image_pooling_BN', epsilon=1e-5)(aspp4)
        ...
        with tf.variable_scope("decoder"):
            # x4 (x2) block
            x = layers.Lambda(lambda s: tf.image.resize_bilinear(s, size=(int(np.ceil(input_shape[0] / 4)), int(np.ceil(input_shape[0] / 4)))), name='UpSampling1')(x)

            dec_skip1 = layers.Conv2D(48, (1, 1), padding='same', use_bias=False, name='feature_projection0')(encoder.get_layer('block3_sepconv2_bn').output)
            ...
            x = layers.Lambda(lambda s: tf.image.resize_bilinear(s, size=K.int_shape(input_tensor)[1:3]), name='UpSampling2')(x)
